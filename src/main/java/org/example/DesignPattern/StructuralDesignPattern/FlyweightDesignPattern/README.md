...
The main part of the pattern is to update the memory usage.

In the application like shooting games where many **identical bullets** are fired and the memory issue will appear when the bullets will start storing the redundant data.

The Flyweight pattern will help reduce the memory overhead by sharing the intrinsic properties of the bullets and maintaining an unique extrinsic properties.

The game has 5 layers and each player fire around 10^3 bullets. It will take memory and make the game slow.

In the Flyweight pattern we have to identify the properties which are common in the bullets. Example the bullet color, image and all should not be loaded every time. The velocity of the bullet, x, y position are not fixed for all bullets. 

The main part is to divide the properties which are common for all (intrinsic properties) and which are different(extrinsic properties).

In application like Google Maps the Hotel icons, petrol pump icons and all are same and loading them will take memory.

It is better to share the common state among multiple object the patter reduces the memory footprint.

The solution is to use the Bullet to store the extrinsic propertu and the intrinsic data like the color will be stored in the BulletType class
